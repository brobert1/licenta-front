    ...
    <div className="mt-4 space-y-2">
      {allHeaders.map(([key, value]) => (
        <div key={key} className="flex items-center gap-4">
          <label className="text-muted text-sm font-medium w-32 flex-shrink-0">{key}</label>
          <Input
            value={value || ''}
            onChange={(e) => handleHeaderChange(key, e.target.value)}
            className="flex-1 bg-tertiary text-primary border border-border rounded px-2 py-1 text-sm focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent/20"
          />
        </div>
      ))}
    </div>
