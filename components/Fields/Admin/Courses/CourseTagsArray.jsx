...
const CourseTagsArray = ({ placeholder, charactersLimit, elementLimit, ...props }) => {
  ...
  return (
    <div {...props}>
      ...
      {charactersLimit && (
        <p className="ml-1 mt-1 text-xs text-muted">{`${newValue.length}/${charactersLimit} characters`}</p>
      )}
      ...
      <div className="mt-3 flex w-full gap-2 overflow-x-auto flex-nowrap scrollbar-none">
        {values.map((value, index) => (
          <Pill
            key={index}
            className="bg-tertiary text-primary hover:cursor-pointer hover:border-border flex-shrink-0"
