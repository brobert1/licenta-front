...
const PgnStudyField = ({ name, ...props }) => {
  ...
  return (
    <div className="flex flex-col gap-3">
      <textarea
        rows="8"
        className="textarea bg-tertiary text-primary border border-border w-full text-sm resize-none"
      ...
      >
        <div className="flex w-full items-center rounded-md border border-border bg-tertiary px-4 py-3">
          <button
            type="button"
            className="px-4 py-2 bg-secondary hover:bg-tertiary rounded text-primary font-medium transition-colors border border-border"
          >
            Choose File
          </button>
          <span className="ml-4 text-muted">{truncate(fileName, 30) || 'No file chosen'}</span>
        </div>
      </FileUploader>
    </div>
  );
