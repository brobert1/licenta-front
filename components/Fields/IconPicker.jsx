...
const IconPicker = ({ onChange, value }) => {
  ...
  return (
    <div className="relative w-full" ref={pickerRef}>
      <div onClick={handleInputClick} className="dropdown cursor-pointer px-4 py-1.5">
      ...
      {isOpen && (
        <div className="fixed inset-x-4 mt-2 bg-tertiary border border-white/10 rounded-lg shadow-lg z-50 max-h-96">
            ...
            {fontAwesomeIcons.map((icon) => (
              <button
                ...
                className={classnames(
                  ...
                  selectedIcon === icon && 'border-accent bg-blue-50 text-accent',
                  selectedIcon !== icon &&
                    'border-white/10 hover:border-white/20 text-white hover:bg-white/10'
                )}
                title={icon}
                type="button"
              >
