...
const FeedbackIcon = ({ feedback, lastMove, firstTurn }) => {
  if (!feedback || !lastMove) {
    return null;
  }

  // Get the destination square of the last move
  const targetSquare = Array.isArray(lastMove) ? lastMove[1] : lastMove.to;
    ...
    <>
      {feedback === 'success' && (
        <div className={classnames('top-right w-[14%] h-[14%]', leftClass, bottomClass)}>
          <div className="relative w-4 h-4 md:w-5 md:h-5 2xl:w-6 2xl:h-6 animate-feedback-in rounded-full">
