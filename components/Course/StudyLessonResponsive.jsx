import { StudySkeleton } from '@components/Study';
import { useWindowSize } from '@hooks';
import StudyLessonContent from './StudyLessonContent';

const StudyLessonResponsive = ({ prevVideoUrl, status, layoutType, layoutProps }) => {
  const { isReady, isMobile, isMd, is2xl } = useWindowSize();

  if (status === 'loading') return <StudySkeleton type="loading" />;
  if (status === 'error') return <StudySkeleton type="error" />;
  if (!isReady) return <StudySkeleton type="loading" />;

  return (
    <>
      {isMobile && (
        <div className="w-full">
          <StudyLessonContent
            size="small"
            layoutType={layoutType}
            layoutProps={layoutProps}
            prevVideoUrl={prevVideoUrl}
          />
        </div>
      )}
      {isMd && (
        <div className="w-full">
          <StudyLessonContent
            size="medium"
            layoutType={layoutType}
            layoutProps={layoutProps}
            prevVideoUrl={prevVideoUrl}
          />
        </div>
      )}
      {is2xl && (
        <div className="w-full">
          <StudyLessonContent
            size="large"
            layoutType={layoutType}
            layoutProps={layoutProps}
            prevVideoUrl={prevVideoUrl}
          />
        </div>
      )}
    </>
  );
};

export default StudyLessonResponsive;
