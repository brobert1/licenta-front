...
const ViewReviewDetailsModal = ({
  ...
  return (
    <Modal show={isOpen} onHide={hide} backdrop="static" keyboard={false} centered>
      ...
      <Modal.Body>
        <div className="mb-4">
          <h3 className="font-semibold text-lg text-white">{data.user.name}</h3>
          <StarRating value={data.rating} readOnly={true} />
        </div>
        <p className="font-bold text-white">{data.name}</p>
        <p className="text-white">{data.review}</p>
      </Modal.Body>
      <Modal.Footer className="flex justify-end">
        <div className="flex gap-2">
          <Button
            className="button full primary delete flex items-center gap-2"
