...
const usePgnViewer = (pgn, options) => {
  ...
  const [currentMoment, setCurrentMoment] = useState(
    startAtLastMove ? lastMoment || defaultMoment : firstMoment || defaultMoment
  );
  ...
  useEffect(() => {
    ...
    if (startAtLastMove) {
      ...
      const newLastMoment = newMoments[newMoments.length - 1];
      if (newLastMoment) {
        setCurrentMoment(newLastMoment);
        setFen(newLastMoment.fen);
      }
    }
  }, [pgn, startAtLastMove]);
